...
import com.paypal.selion.elements.IOSSeLionElementSet.IOSSeLionElement;
...
public class IOSSeLionElementSetTest {
    @BeforeClass
    public void before() {
    ...
    @Test
    public void testGet() {
        SeLionElementSet elementSet = IOSSeLionElementSet.getInstance();
        assertNotNull(elementSet);
        // check for gte due to other tests adding custom elements to the set
        assertTrue(elementSet.size() >= 11);
        assertTrue(elementSet.contains(IOSSeLionElement.UIA_BUTTON));
    }
}
