...
if [ -n "${SAUCE_USERNAME}" ]; then
  ...
  mvn test -Djava.net.preferIPv4Stack=true -DSELION_BROWSER_CAPABILITY_PLATFORM=WINDOWS -B -V
else
  mvn test -Djava.net.preferIPv4Stack=true -DsuiteXmlFile=PhantomJS-Suite.xml -DSELION_SELENIUM_RUN_LOCALLY=true -B -V
fi
