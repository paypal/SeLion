...
java -DarchiveHome=$PROJECT_HOME -DSeLionConfig=$UNIX_CONFIG_PATH/SeLionConfig.json -cp $JAR_DIR/*:. com.paypal.selion.utils.JarSpawner  "java -DarchiveHome=$PROJECT_HOME -DSeLionConfig=$UNIX_CONFIG_PATH/SeLionConfig.json -Djava.util.logging.config.file=$CONFIG_PATH/logging.properties -cp $JAR_DIR/*:. com.paypal.selion.grid.SeLionGridLauncher -role node -hub $HUB_URL -port 5556 -nodeConfig $UNIX_CONFIG_PATH/nodeConfig.json -browserTimeout 60 -servlets com.paypal.selion.node.servlets.NodeAutoUpgradeServlet,com.paypal.selion.node.servlets.NodeForceRestartServlet,com.paypal.selion.node.servlets.LogServlet"