...
start "SeLion Grid Node connected to %HUB_URL%" java -DarchiveHome=%archiveDir% -DSeLionConfig=%WIN_CONFIG_PREFIX%SeLionConfig.json -cp %jarDir%/*;. com.paypal.selion.utils.JarSpawner  "java -DarchiveHome=%archiveDir% -DSeLionConfig=%WIN_CONFIG_PREFIX%SeLionConfig.json -Djava.util.logging.config.file=%archiveDir%/config/logging.properties -cp \"%jarDir%/*;.\" com.paypal.selion.grid.SeLionGridLauncher -role node -hub %HUB_URL% -port 5556 -nodeConfig %WIN_CONFIG_PREFIX%nodeConfig.json -browserTimeout 60 -servlets com.paypal.selion.node.servlets.NodeAutoUpgradeServlet,com.paypal.selion.node.servlets.NodeForceRestartServlet,com.paypal.selion.node.servlets.LogServlet"